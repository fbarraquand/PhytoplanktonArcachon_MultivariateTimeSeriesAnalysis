...
scenario=as.character(1)
BFGS=FALSE

###Loading data
filename=paste("MockPlanktonTimeSeries_Loop.csv",sep="")
...
for (r in 1:iter_scenario){
	...
	print(paste("Lauching bootstrap for scenario",scenario,"repetition",r,"kem method no season",sep=" "))
	...
	print(paste("Saving scenario",scenario,"repetition",r,"kem method no season",sep=" "))
	
	#verif BFGS
	if(BFGS){
        fit_log=MARSS(tab_sp, method="BFGS",model=model.list)
	print(paste("Lauching bootstrap for scenario",scenario,"repetition",r,"BFGS method no season",sep=" "))
        cis=MARSSparamCIs(fit_log,method="parametric",nboot=iter_boot,alpha=aalpha)
        save(fit_log,cis,file=paste("MockSeries_scenario",scenario,"_rep",r,"_bfgs_noseason.RData",sep=""))
	print(paste("Saving scenario",scenario,"repetition",r,"BFGS method no season",sep=" "))
	}
	...
	tab_cov=t(scale(tab_cov_bis))
        ...
        model.list=list(B=B1,U=U1,Q=Q1,Z=Z1,A=A1,R=R1,x0=pi1,V0=V1,tinitx=1,C=C1,c=c1)
        fit_log=MARSS(tab_sp, method="kem",model=model.list,control=cntl.list)
	print(paste("Lauching bootstrap for scenario",scenario,"repetition",r,"kem method with season",sep=" "))
        cis=MARSSparamCIs(fit_log,method="parametric",nboot=iter_boot,alpha=aalpha)
        save(fit_log,cis,file=paste("MockSeries_scenario",scenario,"_rep",r,"_kem_season.RData",sep=""))
	print(paste("Saving scenario",scenario,"repetition",r,"kem method with season",sep=" "))

	if(BFGS){
        #verif BFGS
        fit_log=MARSS(tab_sp, method="BFGS",model=model.list)
	print(paste("Lauching bootstrap for scenario",scenario,"repetition",r,"BFGS method with season",sep=" "))
        cis=MARSSparamCIs(fit_log,method="parametric",nboot=iter_boot,alpha=aalpha)
        save(fit_log,cis,file=paste("MockSeries_scenario",scenario,"_rep",r,"_bfgs_season.RData",sep=""))
	print(paste("Saving scenario",scenario,"repetition",r,"BFGS method with season",sep=" "))
	}
}
